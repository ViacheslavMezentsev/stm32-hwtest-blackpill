...
## HW_CLOCK
После инициализации SystemCoreClock равен 8000000, SYSCLK использует PLL от HSI (64 МГц),
AHB имеет делитель 8, APB1/APB2 — 1 (PCLK1=PCLK2=8 МГц).

## HW_GPIO
Включено тактирование GPIOG. PG13 настроен как push-pull output,
...
## HW_GPIO_ARGUMENTS
Вызов HAL_GPIO_Init для GPIOG получает PG13, push-pull output, no-pull,
low-speed. После инициализации MODER13 подтверждает режим output.

## HW_GPIO_FILTERED_CALL
Условная остановка выбирает вызов HAL_GPIO_TogglePin для PG13 с ODR13=1,
...
делитель PCLK2/2 (ADC=4 МГц), scan без continuous. DMA2 Stream0 channel0 настроен
...
## HW_RTC_INIT
RTC тактируется от готового LSI, делители равны 127/249, внешний выход выключен.
...
(м°C) и происхождение параметров: F429 factory. Широкие диапазоны
...
F429 заводские точки 30°C/110°C при 3300 мВ.
Это проверка арифметики и публикации, не точности физических датчиков.

## HW_SLEEP_SYSTICK
Idle вызывает HAL Sleep/WFI с активным SysTick, SLEEPDEEP сброшен; наблюдается
SysTick handler и возврат в loop после 500 мс HAL-времени. Публикация ADC сохранена.
Остановка на функции не доказывает физический Sleep: отдельно наблюдать S_SLEEP.

## HW_SLEEP_TIMER
При временно отключённом SysTick interrupt наблюдается TIM2 callback в idle;
после восстановления SysTick приложение возвращается к loop. Это проверка
доставки IRQ/продолжения работы, а не единственности источника пробуждения (RTC активен).
