...
проект-потребитель (build/offline); пакет не опубликован, imports/CLI/CMake ещё
...
- Build/offline подключение потребителя проверено; проверить полный аппаратный lifecycle, включая recovery.
...
checkout. Копирование в сторонний каталог и установка пакета пока не проверялись.
...
ELF regression. Аппаратные результаты прежних этапов не являются проверкой нового
consumer lifecycle; в этом этапе отладчики не использовались.
...
- Locks сейчас project-local: запретить параллельный доступ разных потребителей
  к одному отладчику организационно; перед независимой поставкой выбрать общий
  координатор ресурсов без записи внутрь зависимости.
- Доказать отсутствие записей в dependency checkout при полном HW lifecycle,
  переносе модуля и recovery. Вспомогательные experiment/observe_sleep скрипты
  основного проекта ещё не являются переносимым API модуля.
- Проверить consumer на F411/ST-Link, затем восстановить основную прошивку и
  повторить её smoke checks. Стенд не нужно переподключать; перед запуском объявить его.
