"""Explicit PoC runner, not a new production backend or a replacement STM32 runner."""
import argparse
...
def main():
    ...
    p.add_argument("stage", choices=("offline", "flash", "verify", "negative", "stall"))
    p.add_argument("--stand", type=Path)
    p.add_argument("--build", type=Path, default=ROOT / "build/k1921vg015-poc")
    ...
    elf, image = build / "k1921-blink.elf", build / "k1921-blink.bin"
    ...
    config = dict(stage=a.stage, elf=(out / elf.name).as_posix(), image=(out / image.name).as_posix(),
                  module=MODULE.as_posix(), out=out.as_posix(), regions=manifest["load_regions"])
    ...
    command = [str(a.gdb), "-nx", "-nh", "-batch", "-iex", "set auto-load off",
               "-ex", "source " + (HERE / "gdb_agent.py").as_posix()]
    ...
    if (out / "result.json").exists():
        ...
        print(json.dumps({k: v for k, v in result.items() if k not in ("macros", "stops", "checks")}, indent=2))
        print("Checks:", len(result.get("checks", [])))
    else:
