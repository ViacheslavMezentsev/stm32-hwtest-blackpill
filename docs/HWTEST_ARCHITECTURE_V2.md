...
Срез реализации: коммит `ad6f4c1` (2026-09-23). Обозначение **v2 относится к документу**,
...
| --- | --- |
| `stm32_config.yml` | Общая конфигурация сборки с профилями f103/f411 |
...
| `Tests/contracts.json` внутри профиля | Schema 1 предварительных ELF/HAL-проверок; заполнена для семи сценариев F103 |
...
NULL-инъекции RCC F103 привязаны к хешу рассмотренного HAL-исходника; изменение
...
| F411, WeAct BlackPill V3.1, LED PC13 | ST-Link / OpenOCD | Исходные 17 HW-сценариев; новые ADC/Sleep/manifest/HAL ещё не проверены |
| F411 | J-Link либо ST GDB Server | Пока не проверено; mapping J-Link ещё нужно добавить |
...
проверяет семь контрактов (80 проверок) и шесть намеренно неверных вариантов;
...
Актуальная последовательность без сроков — [TODO](../TODO.md). На этом срезе:

1. **F411 + J-Link после подтверждения смены платы.** Добавить device mapping,
   проверить сборку/manifest, адаптировать контракты к фактическому HAL F4 и повторить
...
2. **Закрыть матрицу серверов.** F411 + ST-Link через OpenOCD/ST server, затем
   регрессия новых механизмов на F103 + ST-Link. Критерий: одинаковый ELF внутри
