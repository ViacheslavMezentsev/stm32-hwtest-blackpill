...
- По явному запросу владельца выполнен эксперимент неизменённого f401cc на DEV_ID0x431 через Tests/experiments/f401_marking.py: обычный target не меняется, runtime override отражён в отчётах. Три init PASS; boot/clock timeout из-за двух setup в main (повторный запуск TIM2, HAL_TIM_STATE_BUSY). Дубли setup/loop пока сохранены для проверки «как есть». Сейчас приложение после reset попадает в Error_Handler; это не halt отладчика. Следующий шаг: исправить дубли, новый ELF и повторный эксперимент. См. docs/F401_MARKING_EXPERIMENT.md.
