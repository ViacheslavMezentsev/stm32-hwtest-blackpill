...
> **Важно:** код в приложениях и командные строки пробников — проектные скелеты, они не запускались на железе. Флаги внешних утилит нужно сверить с `--help` установленных версий (см. раздел 9).
...
6. [Пробники: J-Link, ST-Link, OpenOCD](#6-пробники-j-link-st-link-openocd)
...
3. **Бэкенды пробников** (`jlink` / `stlink` / `openocd`) прячут различия серверов и диалектов `monitor`.
...
| P11 | Нет блокировки стенда, `ctest -j` сломает стенд (общие порты и пробники) | `add_test` без `RESOURCE_LOCK` | Средняя |
...
| **Независимость от пробника** | Один тест работает на J-Link и ST-Link. Различия только в бэкенде |
| **Независимость от ОС** | Оркестратор на Python, без `.cmd`. Windows / Linux / macOS |
| **Конфигурация ≠ код** | Пробники, серийные номера, порты — в описании стенда, не в скриптах и не в CMake |
...
flowchart TB
  ...
  subgraph A["Аддитивный слой hwtest/ (подмодуль)"]
    ...
    PRB["Бэкенды пробников:<br/>openocd · jlink · stlink"]
...
| ------ | ----------- | -------------- |
| CMake-слой | ELF-цель, список тестов, метки, таймауты | Пробники, серийные номера |
| Оркестратор | Стенд, пробники, жизненный цикл процессов | Логику конкретных тестов |
| Бэкенд | Командные строки и диалект `monitor` одного пробника | Проект, тесты |
...
sequenceDiagram
  ...
  participant PR as Бэкенд пробника
...
Стенд — это набор **узлов** (`node` = один МК под одним пробником). Тест объявляет, на каких узлах он выполняется (как сегодняшний `for %%n in (1,2)`). Пробники могут быть **разными** на разных узлах.
...
| Идентификация пробника | `-select USB=<SN>` | опция серийного номера (см. `--help`) | `adapter serial <SN>` |
| `release()` | `JLink.exe` / `JLinkExe` (`r; g; q`) | `STM32_Programmer_CLI` (сброс) либо GDB-подключение | `openocd -c "init; reset run; shutdown"` |
| Дополнительно | RTT, Flash Breakpoints (лицензия) | нужна STM32CubeProgrammer | RTT через `rtt …`, единый бэкенд для обоих пробников |
...
Один и тот же путь `load` + `compare-sections` работает для всех пробников через GDB-протокол, поэтому различий по прошивке между J-Link и ST-Link нет.
...
- **Параллелизм:** `RESOURCE_LOCK` защищает единственный стенд. Оркестратор дополнительно берёт файловую блокировку по серийному номеру пробника (защита от запусков вне CTest). Для парка стендов позже можно перейти на `RESOURCE_GROUPS`/`--resource-spec-file` (CMake ≥ 3.16).
...
### 5.5. Presets (без пересборки при смене пробника)

Пробник и стенд выбираются **в момент запуска тестов**, не при конфигурации:
...
## 6. Пробники: J-Link, ST-Link, OpenOCD
...
| --- | --- | --- | --- |
| Доступность нескольких пробников | по SN | по SN | Порты выдавать динамически, а не фиксировать `2331/2334` |
...
| Клоны ST-Link | — | возможны сбои SWD | Для стенда лучше оригинал, обновить прошивку пробника |

Обнаружение пробников (для мастера создания стенда): `hwtest probes list` опрашивает `JLink.exe` (`ShowEmuList`), `STM32_Programmer_CLI -l st` и OpenOCD, печатает серийные номера в формате `[[node]]`.
...
| Драйвер WinUSB для OpenOCD на Windows | Может отобрать пробник у вендорских утилит | Пробный запуск на выделенной машине |
...
### A. Бэкенд пробника (базовый класс и J-Link)
...
class Target:
    ...
    def boot_to(self, sym="main"):
        for c in self.cmds.reset_halt:           # диалект пробника
            gdb.execute(c)
...
hwtest probes  list                                    # обнаружение пробников
```
