...
- Build/offline и аппаратный lifecycle потребителя проверены; проверить перенос checkout и read-only dependency.
...
- Проверить перенос модуля и read-only dependency checkout; текущий lifecycle
  подтверждён сравнением файлов до/после, без трассировки всех операций. Вспомогательные experiment/observe_sleep скрипты
