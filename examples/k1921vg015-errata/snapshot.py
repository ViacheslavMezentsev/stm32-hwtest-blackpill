...
if __name__ == "__main__":
    ...
    p.add_argument("--ram", choices=("ON", "OFF", "TRUE", "FALSE"), required=True)
    a = p.parse_args()
    ...
    symbols = subprocess.check_output([str(a.objdump), "-tC", str(a.elf)], text=True)
    table = [line for line in symbols.splitlines() if "::powers_of_ten_float" in line]
    if len(table) != 1:
        raise ValueError("Expected exactly one powers_of_ten_float table")
    address = int(table[0].split()[0], 16)
    lower, upper = (0x40000000, 0x40040000) if a.ram in ("ON", "TRUE") else (0x80000000, 0x80100000)
    if not lower <= address < upper:
        raise ValueError("from_chars table is not in the intended memory region")
    library = Path(subprocess.check_output([str(a.compiler), "-march=" + a.arch,
        "-mabi=ilp32f", "-print-file-name=libstdc++.a"], text=True).strip()).resolve()
    ...
    report.update(table_address=address, table_in_ram=a.ram in ("ON", "TRUE"),
                  library_sha256=digest(library), library=str(library), arch=a.arch)
