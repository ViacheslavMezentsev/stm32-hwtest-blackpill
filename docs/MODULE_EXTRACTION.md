...
- Build/offline, аппаратный lifecycle, перенос исходного дерева и read-only dependency проверены; решить межпроектное владение отладчиком.
...
Блокировка пока project-local: следующим этапом нужен общий механизм для разных
потребителей, учитывающий один ST-Link при разных backend и освобождение после сбоя.
