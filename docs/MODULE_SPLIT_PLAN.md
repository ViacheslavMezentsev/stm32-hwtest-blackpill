...
## Разделение документации после подключения

Механизмы перенесены в отдельный модуль; приложение теперь стенд его проверки.
[Карта владельцев](README.md), [границы](MODULE_EXTRACTION.md), [артефакты](BUILD_ARTIFACTS.md).
Код ядра и firmware в этом этапе не менялись. Git checkout разработки хранится отдельно
от build в .work; публикация коммита модуля должна предшествовать push gitlink проекта.
Коммит документации модуля: `60eaaf4`; локальный checkout `.work/stm32-gdbtest`, ветка main.
Gitlink потребителя обновлён локально. Проверены файловые Markdown-ссылки и отсутствие
изменений runtime/firmware; аппаратная регрессия предыдущего этапа не переобозначается
как новый прогон. Очистка: семь устаревших каталогов, 124005883 байта до сохранения
диагностики; локальная опись — build/cleanup-evidence/removed.json.
