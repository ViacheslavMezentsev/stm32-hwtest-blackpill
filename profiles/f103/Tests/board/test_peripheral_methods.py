"""Methods derived from BBC_SW_LLR, exercised on the BluePill application without test hooks."""
...
def gpio_arguments(t):
    ...
    t.reach("loop")
    t.check("PC13 mode applied", (t.value("GPIOC->CRH") >> 20) & 15, 2)


@case("HW_GPIO_FILTERED_CALL", labels=("gpio", "contract"))
def gpio_filtered_call(t):
    # First call sees ODR13=1. Select the second call, after the first toggle.
    t.reach("HAL_GPIO_TogglePin",
            when="GPIOx == GPIOC && GPIO_Pin == GPIO_PIN_13 && (GPIOC->ODR & GPIO_PIN_13) == 0")
    t.check("ODR before selected toggle", (t.value("GPIOC->ODR") >> 13) & 1, 0)
    t.reach("loop")
    t.check("ODR after selected toggle", (t.value("GPIOC->ODR") >> 13) & 1, 1)
