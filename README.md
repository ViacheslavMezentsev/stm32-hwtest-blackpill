...
Сборка восстановлена и проверена без платы. Прошивка переключает PC13 каждые
500 мс (полный период — около 1 с), использует HSI 16 МГц, APB1/APB2 8 МГц.
Semihosting не требуется; UART/VCOM и HW-раннер пока не реализованы.
Работа на реальном оборудовании ещё не проверена.
...
настройку `toolchain` VS Code. Профиль OpenOCD подготовлен, но с пробником ещё не проверен.
